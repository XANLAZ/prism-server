...
- **Docker**：在 `docker-compose-env-full.yaml` 中挂载；需根据本机 Docker 的容器日志路径（如 `/var/lib/docker/containers`）正确配置。
...
由 `docker-compose-env-full.yaml` 启动。默认暴露端口：Elasticsearch 9200，Kibana 5601。
...
- **Docker 日志路径**：Filebeat 需能读到容器日志文件；`filebeat.yml` 中的路径需与 Docker 的 containers 目录一致（如 `/var/lib/docker/containers`）。若 Docker 根目录不同，需在 `docker-compose-env-full.yaml` 中调整挂载。
- **go-stash 镜像**：使用 `docker-compose-env-full.yaml` 中指定的镜像（如 `kevinwan/go-stash:1.1.1`）。若配置格式不兼容，请参考 [go-stash](https://github.com/kevwan/go-stash) 文档。
